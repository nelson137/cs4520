...
TEST(b_tests, file_creation_two) {
	...
	// 255 inodes, we need to make ~9 folders to contain all that, not good.
	// so we'll make dirs /[a-o]/[a-o] and that will get us to 240 inodes
	// so we need to fill 15 directories
	for (char dir = 'a'; dir < 'p'; ++dir) {
		fname[1] = dir;
		fname[2] = '\0';
		//		printf("File: %s\n", fname);
	...
	// CREATE_FILE 19
	ASSERT_LT(fs_create(fs, "/a/o", FS_DIRECTORY), 0);
	// Start making files
	// this should fill out /[a]/[a]/[a-o]
	fname[1] = 'a';
	fname[2] = '/';
	fname[3] = 'a';
	fname[4] = '/';
...
TEST(d_tests, write_file_simple) {
	...
	// FS_WRITE 1
	ASSERT_EQ(fs_write(fs, fd_array[0], three_a, 600), 600);

	// FS_WRITE 2
	ASSERT_EQ(fs_write(fs, fd_array[0], large_eight_five_b_seven, 900), 900);

	// file should be 333 0x33, 267 0xAA, 888 0x88 , 12 0x55 and dipping into second block
	// FS_WRITE 3
	ASSERT_EQ(fs_write(fs, fd_array[1], two_nine, 1024), 1024);

	// FS_WRITE 4
	ASSERT_EQ(fs_write(fs, fd_array[1], two_nine + 1024, 1024), 1024);
...
TEST(d_tests, write_file_fill) {
	...
	// total blocks			65536
	// eaten by free bitmap	8
	// eaten by overhead		17
	// one data block for root dir		1
	// direct inodes	will eat		6
	// indirect inodes	eat		1 + 512
	// how many left for dbindirect inodes?
	// 65536 - 8 - 17 - 1 - 6 - (1+512) = 64991
	// Okay, the dbindirect block eats one more for the addressing block,
	// 64991 - 1 = 64990
	// Got the pattern after trying some numbers!
	// 64990 = 513 * 126 + 1 + 351		careful about the calculations here!
	// so 127 double indirects, and the 127th double indirect pointer points to a block which only 351 addresses are used
	...
	uint8_t *giant_data_chunk = new (std::nothrow) uint8_t[1024 * 512]; // an entire indirect block
	ASSERT_NE(giant_data_chunk, nullptr);
	memset(giant_data_chunk, 0x6E, 1024 * 512);

	// now try to fill up all of the rest blocks
	for (int i = 0; i < 126; ++i, blocks += 513) {
		ASSERT_EQ(fs_write(fs, fd, giant_data_chunk, 1024 * 512), 1024 * 512);
	}
	...
	// According to my investigation, there's 351 blocks left
	ASSERT_EQ(fs_write(fs, fd, giant_data_chunk, 1024 * 512), 1024 * 351);
	blocks += 351;
...
TEST(e_tests, remove_file) {
	...
	ASSERT_EQ(system("cp d_tests_full.FS e_tests_a.FS"), 0);
...
TEST(g_tests, seek) {
	...
	ASSERT_EQ(system("cp d_tests_full.FS g_tests.FS"), 0);
	FS *fs = fs_mount(test_fname);
	ASSERT_NE(fs, nullptr);
...
TEST(h_tests, read) {
	...
	ASSERT_EQ(memcmp(write_space, large_eight_five_b_seven + 444, 1000), 0);
	// FS_READ 3
	...
	// FS_READ 6
	ASSERT_EQ(fs_seek(fs, fd, (512 + 6) * 1024, FS_SEEK_SET), (512 + 6) * 1024);
	...
	ASSERT_EQ(fs_seek(fs, fd, (6 + 512 + 126 * 512 + 349) * 1024, FS_SEEK_SET), (6 + 512 + 126 * 512 + 349) * 1024);
	nbyte = fs_read(fs, fd, &write_space, 2048);
	ASSERT_EQ(nbyte, 2048);
	...
	ASSERT_EQ(fs_seek(fs, fd, 0, FS_SEEK_CUR), (6 + 512 + 126 * 512 + 351) * 1024);
	// FS_READ 9
...
TEST(j_tests, link) {
	...
	ASSERT_LT(fs_remove(fs, "/folder2"), 0);    // Cannot be deleted, hardlink corresponds to inode, empty directory should not be deleted
